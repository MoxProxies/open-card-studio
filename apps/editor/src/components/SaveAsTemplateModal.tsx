...
import { X, Loader2, Lock, PencilLine, Unlock } from "lucide-react";
...
export function SaveAsTemplateModal({ design, existing, onSaved, onClose }: SaveAsTemplateModalProps) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Couldn't save the template — check your connection and try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div
      ...
      style={{ position: "fixed", inset: 0, background: "var(--cs-backdrop)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1001 }}
    >
      <form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid var(--cs-border)" }}>
          <h2 className="cs-heading" style={{ fontSize: 16, fontWeight: 600, margin: 0, flex: 1 }}>
            {existing ? "Update template" : "Save as template"}
          </h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: 16 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: "var(--cs-text-muted)" }}>
            Name
            <input className="cs-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Template name" autoFocus data-testid="template-name" />
          </label>
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: "var(--cs-text-muted)" }}>
            ...
            <input className="cs-input" value={tags} onChange={(e) => setTags(e.target.value)} placeholder="fantasy, minimal, full-art" data-testid="template-tags" />
          </label>

          <label style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: "var(--cs-text-muted)" }}>
            Visibility
            <select className="cs-input" value={visibility} onChange={(e) => setVisibility(e.target.value as TemplateVisibility)} data-testid="template-visibility">
          ...
          <div style={{ border: "1px solid var(--cs-border)", borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", gap: 6 }} data-testid="template-lock-breakdown">
            <span style={{ fontSize: 12, fontWeight: 600 }}>What people can change</span>
            <span style={{ fontSize: 12, color: "var(--cs-text-muted)", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--cs-text-muted)" }}>
              Lock a layer (and content-lock it) in the layers panel to fix it in your template; leave a locked layer's content unlocked to make it a fill-in slot.
            </span>
          </div>

          {visibility === "published" && (
            <p style={{ fontSize: 11, color: "var(--cs-text-muted)", margin: 0, lineHeight: 1.5 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", padding: "12px 16px", borderTop: "1px solid var(--cs-border)" }}>
